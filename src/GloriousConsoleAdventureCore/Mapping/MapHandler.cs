...
namespace GloriousConsoleAdventure.Mapping
{
    public class MapHandler
    {
        ...
        {
            var map = new Map(mapWidth, mapHeight, percentWalls = 40, randomBlocks = null, mapPalette = Palettes.Cave, emptyMap = false);
            if (emptyMap)
            {
                percentWalls = 0;
            }
            RandomFillMap(map, percentWalls, mapWidth, mapHeight);
            if (!emptyMap)
                MakeCaverns(map);
        ...
        public void CloneExit(Map exittingMap, Direction exitDirection, Map map)
        {
        ...
        public void GenerateExit(Direction direction, Map map)
        {
            var foundExit = false;
            switch (direction)
            {
        ...
        public Map BlastCrossBombermanStyle(Coordinate coordinate, Map map)
        {
        ...
        public void GenerateRandomExitDirection(Map map, int exits)
        {
        ...
        /// Randomly fills map
        /// </summary>
        public void RandomFillMap(Map map, int wallPercent, int mapWidth, int mapHeight)
        {
            
            map.MapBlocks = new Block[map.MapWidth, map.MapHeight];

            int mapMiddle = 0; // Temp variable
            var startColumn = (mapWidth - map.MapWidth) / 2;
            var startRow = (mapHeight - map.MapHeight) / 2;
            for (int column = 0, row = 0; row < map.MapHeight; row++)
            {
                //Border creation
                for (column = 0; column < map.MapWidth; column++)
